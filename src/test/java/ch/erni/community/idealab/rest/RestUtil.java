package ch.erni.community.idealab.rest;

import ch.erni.community.idealab.configuration.OAuth2ServerConfiguration;
import ch.erni.community.ldap.data.DefaultCredentials;
import org.springframework.http.MediaType;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.util.Base64Utils;

import static org.hamcrest.Matchers.*;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

/**
 * @author rap
 */
public class RestUtil {

	public static String authenticateAndGetAccessToken(MockMvc mvc) throws Exception {
		String authorization = "Basic "
				+ new String(Base64Utils.encode((OAuth2ServerConfiguration.CLIENT_ID + ":" + OAuth2ServerConfiguration.CLIENT_SECRET).getBytes()));
		String contentType = MediaType.APPLICATION_JSON + ";charset=UTF-8";

		DefaultCredentials defaultCredentials = new DefaultCredentials();
		String username = defaultCredentials.getDomainShortName();
		String password = defaultCredentials.getPassword();

		String content = mvc
				.perform(
						post("/oauth/token")
								.header("Authorization", authorization)
								.contentType(
										MediaType.APPLICATION_FORM_URLENCODED)
								.param("username", username)
								.param("password", password)
								.param("grant_type", "password")
								.param("scope", "read write")
								.param("client_id", OAuth2ServerConfiguration.CLIENT_ID)
								.param("client_secret", OAuth2ServerConfiguration.CLIENT_SECRET))
				.andExpect(status().isOk())
				.andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$.access_token", is(notNullValue())))
				.andExpect(jsonPath("$.token_type", is(equalTo("bearer"))))
				.andExpect(jsonPath("$.refresh_token", is(notNullValue())))
				.andExpect(jsonPath("$.expires_in", is(greaterThan(4000))))
				.andExpect(jsonPath("$.scope", is(equalTo("read write"))))
				.andReturn().getResponse().getContentAsString();

		return content.substring(17, 53);
	}

}
